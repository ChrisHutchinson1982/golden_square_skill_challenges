...
RSpec.describe DiaryEntry do
  ...
  context "initialize title method" do
    it "returns title" do
  ...
  context "initialize contents method" do
  ...
  describe "reading time method" do
    it "returns reading time when contents divided wpm is a whole number" do
      entry_1 = DiaryEntry.new("Today", "Test driving a class")
      expect(entry_1.reading_time(1)).to eq 4
    end

    it "rounds up reading time when contents divided wpm is not a whole number" do
      entry_1 = DiaryEntry.new("Today", "Test driving a class system for fun")
      expect(entry_1.reading_time(3)).to eq 3
    end
  ...
  describe "reading_chunk method" do
    it "returns reading_chunk from contents that user can read in a minute" do 
      entry_1 = DiaryEntry.new("Today", "I am feeling great but this sounds tricky!")
      expect(entry_1.reading_chunk(1, 1)).to eq "I"
    end

    it "returns error if wpm is 0" do
      entry_1 = DiaryEntry.new("Today", "Test driving a class system")
      expect { entry_1.reading_chunk(0, 5) }.to raise_error "Error, wpm or reading time cannot be 0"
    end 

    it "returns error if reading time is 0" do
      entry_1 = DiaryEntry.new("Today", "Test driving a class system")
      expect { entry_1.reading_chunk(5, 0)}.to raise_error "Error, wpm or reading time cannot be 0"
    end 
    ...
    it "returns third reading_chunk from contents that user can read in a minute" do
      entry_1 = DiaryEntry.new("Today", "I am feeling great but think sounds tricky!")
      entry_1.reading_chunk(1,1)
      entry_1.reading_chunk(1,1)
      expect(entry_1.reading_chunk(2,1)).to eq "feeling great"
    end
